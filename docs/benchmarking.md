...
> **Целевой каталог графиков G1–G14 + дисциплина метрик** (исчерпывающие метрики
> с обоснованием «зачем» и проверкой достаточности под гипотезу) — в
> [HANDOFF.md](../HANDOFF.md) → «Benchmark & graph catalog» / «Metrics discipline».
> Этот файл — про то, что **уже построено** (текущие конфиги/метрики); HANDOFF — про
> **целевой** набор. Список бейзлайнов ниже расширяется до LMDB / TFRecord / HDF5 /
> HuggingFace / FFCV (план, Tier A формат-матрица).
...
2. **Гибкость / уникальные свойства.** Дать данные под фичи, которых нет у конкурентов: server-side decode, concurrent mutations (план), S3 streaming (план), различные типы данных (images + audio + text уже, video в плане).
...
`real_universal_datasets.yaml` проверяет, что DatasetFS не завязан на изображения: runner запускает lightweight training probes по подготовленным датасетам разных модальностей и пишет отсутствующие источники в `missing.csv`.

Для текстовой модальности в конфиг добавлен `amazon_polarity_slice`: реальный HuggingFace text-classification корпус `fancyzhx/amazon_polarity`, materialized slice `--target-gb 1.5`, payload `.txt`, label из исходного датасета. Команда подготовки: `python -m scripts.datasets.prepare_real_universal amazon_polarity_slice --target-gb 1.5`; запуск всей матрицы: `make bench-real-universal`.
...
- **Imagewoof / Speech Commands / Amazon Polarity text slice** в основном headline-сете — для обобщения вне Imagenette. Real-universal конфиг уже содержит текстовый `amazon_polarity_slice` на 1.5 GiB; headline-конфиги можно расширить отдельной single-dataset строкой при необходимости.
