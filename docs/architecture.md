...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  Go daemon (cmd/fuse_daemon)                                               │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │  HTTP control plane (internal/ipc): /healthz /initialize_loading   │    │
│  │                                     /metrics /debug/pprof/*        │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│  ┌────────────────────────────────────────────────────────────────────┐    │
...
- `data/formats/<ds>/datasetfs/` — собственный формат: tar-шарды + `metadata.jsonl`

Имеется конвертер `cmd/dataset_converter` (Go), которого `prepare_formats`
дёргает для построения DFS-формата из imagefolder-структуры.

### Обучение

1. Запускается daemon: `bin/fuse_daemon --no-mount --root <path>`. Daemon
   читает `metadata.jsonl`, строит `CoreIndex` (in-memory: `FileMap`, `ShardMap`),
   ждёт IPC.
...
|---|---|---|
| `cmd/fuse_daemon` | main: парсит флаги, поднимает Manifest+CoreIndex, стартует IPC, опционально mount FUSE | `main.go` |
...
| `internal/shm` | Allocator 9×110 MB + refcounts через `/tmp/mlfs_refs.bin`; atomic ops для синхронизации с Python | `allocator.go`, `refcount.go` |
...
| `internal/ipc` | HTTP server `:51409`: `/healthz`, `/initialize_loading` (создание сессии, поддерживает re-init), `/metrics` (JSON-метрики), `/debug/pprof/*` | `startup_server.go` |
...
- **`cmd/fuse_daemon/main_test.go`** вызывает `main()`, который никогда не возвращается. **Не запускать `go test ./...`** — повиснет. Использовать `go test ./internal/... ./cmd/dataset_converter/...`.
...
```
fuse_daemon --no-mount --root <path>
