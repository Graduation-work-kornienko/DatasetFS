...
| **Optimization track** | Server-side decode (opt 01) → параллельный декод (opt 02) → транспорт пайплайна (opt 03) → ... | ✅ **Opt 01 + 02 + 03 завершены** |
...
### Optimization 03 — транспорт пайплайна (общий путь, не только картинки) ✅ ЗАВЕРШЕНА

Полный контекст: [optimizations/03-pipeline-transport.md](optimizations/03-pipeline-transport.md).

- **Мотивация**: opt 01/02 ускорили только image-декод; для аудио и любых
  «недекодируемых на сервере» типов декод дёшев, и доминирует общий транспорт
  daemon→Python. На image он замаскирован PIL (3.0s/3.7s vs `__iter__` 0.06s).
- **Что сделано**: (1) бинарный length-prefixed wire-фрейм вместо JSON-over-pipe
  (`dealer.go encodeFrame` ↔ `dataset_fs.py`, один векторизованный `np.frombuffer`),
  выкинуты мёртвые поля; (2) zero-copy SHM через `memoryview` (реальный выигрыш на
  rgb_uint8); (3) батч-декремент refcount (одна запись на слот вместо на сэмпл).
- **Попутно закрыт slot-leak**: пропущенные сэмплы (decode→None/mismatch/исключение)
  не декрементили refcount → слот не переиспользовался. Регрессия:
  `tests/test_pipeline_leak.py`.
- **Измерение**: добавлен аудио-бенч (Speech Commands, soundfile→log-mel,
  `simplecnn_audio`) — `make bench-audio`, конфиг `audio_speech_commands.yaml`
  (он же график G8). Микробенч транспорта: `profiling/raw_transport_bench.py`.
- **Результат**: чистый транспорт (num_workers=0) **144.5k → ~205k sps (1.4×)**;
  реальный аудио-путь (soundfile) **17.6k → 19.3k sps (+10%)**. На полном e2e-обучении
  узким местом становится compute (mel+модель) — честная граница: opt 03 ускоряет
  transport-bound режим, где живут дешёвые-декод типы.
- **Корректность**: `go-test`+`-race` зелёные; `test-correctness/imagewoof/audio/
  decode/training` зелёные; leak-регрессия зелёная. Пред-существующий
  `test_manifest_imagenette` падает из-за parquet-миграции (jsonl≠parquet), не связан.
